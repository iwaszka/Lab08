...
```{r}
weather_Portland_numeric <- weather_Portland %>%
  mutate(RainToday = ifelse(RainToday == "Yes", 1, 0),
         RainTomorrow = ifelse(RainTomorrow == "Yes", 1, 0)) 

linear_reg() %>%
  set_engine("lm") %>%
  fit(RainTomorrow ~ RainToday, data = weather_Portland_numeric) %>%
  tidy()
...
```{r}
ggplot(data = weather_Portland_numeric, aes(x = RainToday, y = RainTomorrow)) +
  geom_point() +
...
```{r}
summary(weather_Portland_numeric)
```
